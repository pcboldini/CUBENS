...
module decomp_2d_fft
   ...
   public :: decomp_2d_fft_init, decomp_2d_fft_3d, decomp_2d_fft_1d, &
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine r2c_1m_y_plan(plan1, decomp_ph, decomp_sp)
    ...
    sz = decomp_ph%ysz(1)*decomp_ph%ysz(2)
    a1_p = fftw_alloc_real(sz)
    call c_f_pointer(a1_p,a1, &
         [decomp_ph%ysz(1),decomp_ph%ysz(2)])
    sz = decomp_sp%ysz(1)*decomp_sp%ysz(2)
    a2_p = fftw_alloc_complex(sz)
    call c_f_pointer(a2_p,a2, &
         [decomp_sp%ysz(1),decomp_sp%ysz(2)])

#ifdef DOUBLE_PREC
    plan1 =  fftw_plan_many_dft_r2c(1, decomp_ph%ysz(2), &
         decomp_ph%ysz(1), a1, decomp_ph%ysz(2), &
         decomp_ph%ysz(1), 1, a2, decomp_sp%ysz(2), &
         decomp_sp%ysz(1), 1, plan_type)
#else
    plan1 =  fftwf_plan_many_dft_r2c(1, decomp_ph%ysz(2), &
         decomp_ph%ysz(1), a1, decomp_ph%ysz(2), &
         decomp_ph%ysz(1), 1, a2, decomp_sp%ysz(2), &
         decomp_sp%ysz(1), 1, plan_type)
#endif
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine r2c_1m_y(input, output)
    ...
    do k=1,s3
#ifdef DOUBLE_PREC
    call fftw_execute_dft_r2c(plan(0,2), input(:,:,k), output(:,:,k))
#else
    call fftwf_execute_dft_r2c(plan(0,2), input(:,:,k), output(:,:,k))
#endif    
    enddo
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine fft_1d_r2c(in_r, out_c, fftdir)
    ...
    complex(mytype), dimension(:,:,:), intent(OUT) :: out_c
    integer :: fftdir
    TYPE(DECOMP_INFO) :: spnew

    if (fftdir==1) then
       ! ===== 1D FFTs in X =====
       call r2c_1m_x(in_r,out_c)
                
    else if (fftdir==3) then
       ! ===== 1D FFTs in Z =====
       call r2c_1m_z(in_r,out_c)

    else if (fftdir==2) then
       ! ===== 1D FFTs in Y =====
       call decomp_info_init(nx_global, ny_global/2+1, nz_global, spnew)
       call r2c_1m_y_plan(plan(0,2), ph, spnew)
       call r2c_1m_y(in_r,out_c)
       call decomp_info_finalize(spnew)

    end if
    
    return
  end subroutine fft_1d_r2c
