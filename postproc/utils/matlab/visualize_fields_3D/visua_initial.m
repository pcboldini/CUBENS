...
global visua cut_Re cut_plane

visua='2D'; % 2D/1D
cut_plane='z';  % x/y/z
cut_coord=100; % x_cut/y_cut/z_cut
...
fname = sprintf('../../../restart/ruvwe.0000000.bin');
fileID = fopen(fname);
...
if strcmp(visua,'2D') 
    ...
    Rho_min=min(min(Rho_plane));
    Rho_max=max(max(Rho_plane));
    E_min=min(min(E_plane));
    E_max=max(max(E_plane));
    W_min=min(min(W_plane));
    W_max=max(max(W_plane));
    U_min=min(min(U_plane));
    U_max=max(max(U_plane));
...
elseif strcmp(visua,'1D')
    ...
    elseif strcmp(cut_plane,'x')
      ...
      fprintf('Extract variables at x=%.3f\n',x_cut);
        ...
        Rho_cut(:,:)=rho(index_x_cut,:,:);
        Rho_cut=transpose(Rho_cut);
        E_cut(:,:)=e(index_x_cut,:,:);
        E_cut=transpose(E_cut);
        W_cut(:,:)=w(index_x_cut,:,:);
        W_cut=transpose(W_cut);
        U_cut(:,:)=u(index_x_cut,:,:);
        U_cut=transpose(U_cut);
    ...
    if strcmp(visua,'2D') 
        ...
        colormap(c);  
        hold on
        [~,h] = contourf(plot_coord1,plot_coord2,Rho_plane,'LevelList',linspace(Rho_min-0.1,Rho_max+0.1,nsteps)); hold off
        ...
        colormap(c);  
        hold on
        [~,h] = contourf(plot_coord1,plot_coord2,E_plane,'LevelList',linspace(E_min-0.1,E_max+0.1,nsteps)); hold off
        set(h,'LineColor','none')
        ax=gca;
        ax.TickLabelInterpreter='latex';
        set(gca,'XMinorTick','on','YMinorTick','on')
        ...
        colormap(c);  
        hold on
        [~,h] = contourf(plot_coord1,plot_coord2,W_plane,'LevelList',linspace(0,W_max+0.1,nsteps)); hold off
        set(h,'LineColor','none')
        ax=gca;
        ax.TickLabelInterpreter='latex';
        set(gca,'XMinorTick','on','YMinorTick','on')
        ...
        colormap(c);  
        hold on
        [~,h] = contourf(plot_coord1,plot_coord2,U_plane*1e3,'LevelList',linspace(0,U_max*1e3+0.1,nsteps)); hold off
        set(h,'LineColor','none')
        ax=gca;
        ax.TickLabelInterpreter='latex';
        set(gca,'XMinorTick','on','YMinorTick','on')
