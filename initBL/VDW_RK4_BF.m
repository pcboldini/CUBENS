function[xSol,ySol,CSol,KappaSol,PrSol,RhoSol,TSol] = VDW_RK4_BF(dEqs,y,VDW_PROP,BL_PARA,N,dh)
 % Initial values   
x=0;
...
[CSol(1),KappaSol(1),PrSol(1),RhoSol(1),TSol(1)]=Calc_VDW_Prop(h_ref,VDW_PROP,BL_PARA);
for i=1:N-1
        ...
        k = k+1;
        xSol(k) = x; ySol(k,:) = y;  
        [CSol(k),KappaSol(k),PrSol(k),RhoSol(k),TSol(k)]=Calc_VDW_Prop(ySol(k,4),VDW_PROP,BL_PARA);        
end
end
