...
function [fSol,CSol,KappaSol,PrSol,RhoSol,TSol]=PR_RK4(BL_PARA,PR_PROP,N)
...
[h_rec,~,~,~,~,~,~,~] = PR(T_rec,PR_PROP,BL_PARA);
h_inf=BL_PARA.h_inf;
...
%    [ Cf''   g ]
ic = [0.5    1 ]; %h_rec/h_inf

% Shooting method for initial condition
fprintf('Iteration process: adiabatic wall (Ny=%i) \n',N)
[eta,fSol,CSol,KappaSol,PrSol,RhoSol,TSol,Initialcond] = fsCompAd(ic,BL_PARA,PR_PROP,N,dh);

elseif strcmp(wall_bc,'isoth')
    
% Enthalpy at the wall

[h_wall,~,~,~,~,~,~,~] = PR(BL_PARA.T_wall,PR_PROP,BL_PARA);
...
% Shooting method for initial condition
[eta,fSol,CSol,KappaSol,PrSol,RhoSol,TSol,Initialcond] = fsCompIso(ic,g_w,BL_PARA,PR_PROP,N,dh);

end


function [eta,fSol,CSol,KappaSol,PrSol,RhoSol,TSol,v] = fsCompAd(v,BL_PARA,PR_PROP,N,dh)
...
v = newtonRaphson2(@residual,v,tol,epsilon);
    ...
    function r = residual(v)
        r = zeros(length(v),1);
        [xSol,ySol,CSol,KappaSol,PrSol,RhoSol,TSol] = PR_RK4_BF(@fsCompOde,inCond(v),PR_PROP,BL_PARA,N,dh);
        % Check
        count=count+1;
        lastRow = size(ySol,1);
...
[eta,fSol,CSol,KappaSol,PrSol,RhoSol,TSol] = PR_RK4_BF(@fsCompOde,inCond(v),PR_PROP,BL_PARA,N,dh);


end % fsComp

function [eta,fSol,CSol,KappaSol,PrSol,RhoSol,TSol,v] = fsCompIso(v,g_w,BL_PARA,PR_PROP,N,dh)
