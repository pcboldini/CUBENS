...
!   recycling-rescaling:
    ...
    z_recycle = 155.0_mytype                        ! recycle position
...
!  
    ...
    nsteps           = 400000                       ! simulations steps
    intvCalcCFL      = 10                           ! interval CFL calculation
!
! ---------------------------------- I/O ----------------------------------
!     
    intvPrint        = 10000                        ! on the screen
    readRestartFile  = 2500000                      ! if <0, not read
    intvSaveRestart  = 50000                        ! interval save restart files
    saveRestartAfter = 2450000                      ! save restart files after which timestep
    intvSavePlanes   = 50000                        ! interval save planes
    savePlanesAfter  = 2500000                      ! save planes after which timestep
    intvSaveStats    = 100                          ! interval save statistics
    saveStatsAfter   = 2500000                      ! save statistics after which timestep
    intvReadParam    = -2                           ! read variation file

    yi_plane = (/1/)                                ! y-index cut z-x plane  
    xi_plane = (/0/)                                ! x-index cut z-y plane
    zi_plane = (/0/)                                ! z-index cut y-x plane
!
! ------------------------- FFT (if pert_calc=1) --------------------------         
!
    fft_samples = 20                                ! per period (usually 2 periods enough)
...
!
    ...
    zplus_min = 9.2                                 ! non_equid: zplus in turbulent region 
    z_1 = 0.09                                      ! non_equid: location of first bump in z-direction
    z_2 = 0.98                                      ! non_equid: location of second bump in z-direction
    bumpz1 = 0.1                                    !non_equid: width of first bump in z-direction
    bumpz2 = 0.04                                   ! non_equid: width of second bump in z-direction
    
    zpluspert_min = 20.0                            ! non_equid: zplus in strip region 
    zpert_1 = 0.06                                  ! non_equid: location of first strip bump   
    zpert_2 = 0.14                                  ! non_equid: location of second strip bump  
