...
### Turbulent Channel Flows (Extra)
#### IG
Run the ideal-gas 3-D channel flow at `Re_tau=180` (Kim et al., JFM, 1987) for:
  - `Re=2800, M=0.2, Pr=0.75, dpdx=2.0, EOS_LAW=IG`
...
#### PR
Run the supercritical 3-D channel flow (case TR1) at `Re_tau=180` of Guo et al. (JFM 2022) for:
  - `Re=3000, M=0.178, Pr=2.078, dpdx=3.0, EOS_LAW=PR`
  - `Twall_bot=1.0, Twall_top=1.0, Tref = 0.79, Pref = 1.14`
  - `N_x=129, N_y=160, N_z=192, L_x=2.0, L_y=4.0\pi/3.0, L_z=2.0\pi`

input: `config_CHA.h` file with `Makefile`, for inital condition: `main_IG.ipynb` file generating files in `preproc/initCHA/inputDNS`

output: time- and spanwise-average profiles at `t=1300000` in `stats`, postprocessing script in `postproc.ipynb`


### Differentially Heated Cavity (Extra)
Run the 2-D DHC problem at `Ra=10^6` (Le Quéré et al., ESAIM, 2005) for:
